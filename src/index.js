...
function getConvertionRates(baseRate) {
  return fetch(`${API_URL}latest/${baseRate}`).then((res) => res.json());
}
...
function loadConvertionRates(conversionData, quantity) {
  ...
  Object.entries(conversionData.conversion_rates).forEach(([k, v]) => {
    ...
    <div>
      ...
      ${CURRENCY_DATA[k] && CURRENCY_DATA[k].name}
      </span> 
    </div> 
    <div>
      <span class='currency-symbol'>
      ${CURRENCY_DATA[k] && CURRENCY_DATA[k].symbol.grapheme}
      </span> 
...
function initialize() {
  loadBaseRates($baseRatesSelect, baseRates);

  $FRM.addEventListener('submit', async (event) => {
    event.preventDefault();
    loadConvertionRates(
      await getConvertionRates($FRM.base_rate.value && $FRM.base_rate.value),
      Number($FRM.quantity.value),
    );
  });
